...
class CheckpointConfig(FairseqDataclass):
    load_model_dir: str = field(
        default="checkpoints_default", metadata={"help": "path to load checkpoint from"}
    )
        
    save_model_dir: str = field(
        default="checkpoints", metadata={"help": "path to save checkpoints"}
    )
    load_model_type: str = field(
        default=False, metadata={"help": "if DL_mode is true 6-6 to 3-3, if False, same as original"}
    )        
    ...
    layer_initialization: str = field(
        #default = "checkpoint_best.pt",
        default=False,
        metadata={
            "help": "Needed for layer_initialization"
        },
    )        
    ...
    no_epoch_checkpoints: bool = field(
        default=True, metadata={"help": "only store last and best checkpoints"}
    )
