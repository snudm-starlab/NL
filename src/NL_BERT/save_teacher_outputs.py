...
from utils.utils import count_parameters, eval_model_dataloader, eval_model_dataloader_nli, fill_tensor, load_model, load_model_org
...
encoder_file = os.path.join(HOME_OUTPUT_FOLDER, 'MRPC/BERT12/BERT.encoder_loss.pkl')
cls_file = os.path.join(HOME_OUTPUT_FOLDER, 'MRPC/BERT12/BERT.cls_loss.pkl')

encoder_bert, classifier = init_model(task, output_all_layers, n_layer, config)
encoder_bert = load_model_org(encoder_bert, encoder_file, args, 'exact', verbose=False)
classifier = load_model_org(classifier, cls_file, args, 'exact', verbose=False)
...
if prediction_mode in ['benchmark']:
    ...
    if not output_all_layers:
        ...
        fname = os.path.join(output_dir, task, task + f'_Originalbert_base_kd_normal_kd_teacher_{n_layer}layer_result_summary_2_loss.pkl')
    else:
        #fname = os.path.join(output_dir, task, task + f'_distilbert_patient_kd_teacher_{n_layer}layer_result_summary.pkl')
        fname = os.path.join(output_dir, task, task + f'_Originalbert_base_patient_kd_teacher_{n_layer}layer_result_summary_2_loss.pkl')
