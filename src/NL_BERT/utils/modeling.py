...
class BertForSequenceClassificationEncoder_NL(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, NL_mode = 0):
        if self.output_all_encoded_layers:
            full_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=True, NL_mode = NL_mode)
            return [full_output[i][:, 0] for i in range(len(full_output))], pooled_output
        else:
            _, pooled_output = self.bert(input_ids, token_type_ids, attention_mask, output_all_encoded_layers=False, permutation = permutation, rand_shuffle = rand_shuffle, NL_mode = NL_mode)
            return None, pooled_output
