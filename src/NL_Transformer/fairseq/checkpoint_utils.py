...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss):
    ...
    checkpoints = [
        os.path.join(cfg.save_model_dir, fn) for fn, cond in checkpoint_conds.items() if cond
    ]
    if len(checkpoints) > 0:
        trainer.save_checkpoint(checkpoints[0], extra_state)
        for cp in checkpoints[1:]:
    ...
    if not end_of_epoch and cfg.keep_interval_updates > 0:
        ...
        if cfg.keep_interval_updates_pattern == -1:
            checkpoints = checkpoint_paths(
                cfg.save_model_dir, pattern=r"checkpoint_\d+_(\d+){}\.pt".format(suffix)
            )
        else:
            checkpoints = checkpoint_paths(
                cfg.save_model_dir, pattern=r"checkpoint_\d+_(\d+){}\.pt".format(suffix), keep_match=True
            )
            checkpoints = [x[0] for x in checkpoints if x[1] % cfg.keep_interval_updates_pattern != 0]
    ...
    if cfg.keep_last_epochs > 0:
        # remove old epoch checkpoints; checkpoints are sorted in descending order
        checkpoints = checkpoint_paths(cfg.save_model_dir, pattern=r"checkpoint(\d+){}\.pt".format(suffix))
        for old_chk in checkpoints[cfg.keep_last_epochs :]:
            if os.path.lexists(old_chk):
                os.remove(old_chk)
...
def load_checkpoint(cfg: CheckpointConfig, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    if (
        ...
        checkpoint_path = os.path.join(
            cfg.load_model_dir, "checkpoint_last{}.pt".format(suffix))
...
def load_checkpoint_naive(cfg: CheckpointConfig, trainer, layer_init, **passthrough_args):
    """
    Load a checkpoint and restore the training iterator.
    ...
    *passthrough_args* will be passed through to
    ...
    if (
        ...
        checkpoint_path = os.path.join(
            cfg.load_model_dir, "checkpoint_best{}.pt".format(suffix))        
    ...
    if ( cfg.restore_file != "checkpoint_last.pt"):
        checkpoint_path = os.path.join(
            cfg.load_model_dir, cfg.restore_file.format(suffix))          
    ...
    #optimizer 같은 거 reset 하고 싶으면 아래처럼 true로 바꿔주면 된다.
    #reset_optimizer = True
    #reset_lr_scheduler = True
    #reset_meters = True
    #reset_dataloader = True
    # 그러면 아래의 line 242부터의 코드가 돌아가면 된다. 
    # 여기도 추가했음. 아래 세 줄.
    ...
    extra_state = trainer.load_checkpoint_naive(
        checkpoint_path,
        reset_optimizer,
        reset_lr_scheduler,
        optimizer_overrides,
...
def verify_checkpoint_directory(load_model_dir: str) -> None:
    if not os.path.exists(load_model_dir):
        os.makedirs(load_model_dir, exist_ok=True)
    temp_file_path = os.path.join(load_model_dir, "dummy")
    try:
        with open(temp_file_path, "w"):
            pass
    except OSError as e:
        logger.warning(
            "Unable to access checkpoint save directory: {}".format(load_model_dir)
        )
        raise e
    else:
        os.remove(temp_file_path)
